...
`+JSON.stringify(o,null,2)}catch{u=" "+this.formatData(o)}else u=" "+this.formatData(o);let T="";if(n){let{sessionId:E,memorySessionId:f,correlationId:S,...m}=n;Object.keys(m).length>0&&(T=` {${Object.entries(m).map(([h,b])=>`${h}=${b}`).join(", ")}}`)}let p=`[${i}] [${a}] [${d}] ${c}${s}${T}${u}`;if(this.logFilePath)try{(0,L.appendFileSync)(this.logFilePath,p+`
  ...
  `,a=[t,...e];s!=null&&a.push(s);let d=r.prepare(i).run(...a);return Number(d.changes??0)}function Ue(r,e,t){let s=r.prepare(`
    ...
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return{id:Number(u.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a,d){let c=a??Date.now(),u=new Date(c).toISOString();return this.db.transaction(()=>{let p=[],E=this.db.prepare(`
      ...
      `),f=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let m of s){let A=B(e,m.title,m.narrative),h=E.get(e,t,m.type,m.title,m.subtitle,JSON.stringify(m.facts),m.narrative,JSON.stringify(m.concepts),JSON.stringify(m.files_read),JSON.stringify(m.files_modified),o||null,i,m.agent_type??null,m.agent_id??null,m.agent_tool_id??null,m.visibility??"private",A,u,c,d||null);if(h){p.push(h.id);continue}let b=f.get(e,A);if(!b)throw new Error(`storeObservations: ON CONFLICT without existing row for content_hash=${A}`);p.push(b.id)}let S=null;if(n){let A=this.db.prepare(`
        ...
        ON CONFLICT(memory_session_id, content_hash) DO NOTHING
        RETURNING id
      `),m=this.db.prepare("SELECT id FROM observations WHERE memory_session_id = ? AND content_hash = ?");for(let O of s){let v=B(e,O.title,O.narrative),Se=S.get(e,t,O.type,O.title,O.subtitle,JSON.stringify(O.facts),O.narrative,JSON.stringify(O.concepts),JSON.stringify(O.files_read),JSON.stringify(O.files_modified),a||null,d,O.agent_type??null,O.agent_id??null,O.agent_tool_id??null,O.visibility??"private",v,p,T,u||null);if(Se){f.push(Se.id);continue}let he=m.get(e,v);if(!he)throw new Error(`storeObservationsAndMarkComplete: ON CONFLICT without existing row for content_hash=${v}`);f.push(he.id)}let A;if(n){let v=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).all(...u);if(!i)return E;let f=new Map(E.map(S=>[S.id,S]));return e.map(S=>f.get(S)).filter(S=>!!S)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:n,project:o}=t,i=s==="relevance",a=i?"":`ORDER BY up.created_at_epoch ${s==="date_asc"?"ASC":"DESC"}`,d=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(","),u=[...e],T=o?"AND s.project = ?":"";o&&u.push(o);let E=this.db.prepare(`
      ...
      ${a}
      ${d}
    `).all(...u);if(!i)return E;let f=new Map(E.map(S=>[S.id,S]));return e.map(S=>f.get(S)).filter(S=>!!S)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let m=`
