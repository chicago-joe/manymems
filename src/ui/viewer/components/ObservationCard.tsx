...
export function ObservationCard({ observation, onFileClick }: ObservationCardProps) {
  ...
  const filesModifiedRaw = observation.files_modified ? JSON.parse(observation.files_modified) : [];
      ...
      <div className="card-header">
        <div className="card-header-left">
          ...
          {observation.generated_by_model && (
            <span className="card-model-badge" title={`Generated by ${observation.generated_by_model}`}>
              {observation.generated_by_model.replace(/^claude-/, '').replace(/^gemini-/, 'gemini/').replace(/-\d{8}$/, '')}
            </span>
          )}
          {observation.agent_type && observation.agent_type !== (observation.platform_source || 'claude') && (
            <span className="card-agent-badge" title={`Agent: ${observation.agent_type}`}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
            ...
            {filesModified.length > 0 && (
              <span className="meta-files">
                ...
                {filesModifiedRaw.map((rawPath: string, idx: number) => (
                  <button
                    key={idx}
                    className="file-chip-btn"
                    onClick={() => onFileClick?.({ file: rawPath, line: 1 })}
                    title={`View provenance: ${rawPath}`}
                  >
                    {filesModified[idx]}
                  </button>
                ))}
